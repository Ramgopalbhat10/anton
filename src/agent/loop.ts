...
function systemPrompt(mcpTools: LoadedMcpTools, workspaceRoot?: string): string {
  ...
  return [
    ...
    "- `bash` output streams live to the UI. Run build and test commands raw first so live output is visible; if you need to search or summarize logs, do that in a separate follow-up command after the run completes.",
    ...
    "- Plan first for multi-step tasks: explore read-only tools (`glob`, `grep`, `read_file`) before editing (`edit_file`, `write_file`) or running shell commands.",
